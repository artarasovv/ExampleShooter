...
#include "Tasks/ESAT_WaitTargetDataUsingActor.h"

#include "AbilitySystemComponent.h"
#include "Targets/ESGATA_Trace.h"
#include "Abilities/GameplayAbilityTargetActor.h"


UESAT_WaitTargetDataUsingActor* UESAT_WaitTargetDataUsingActor::WaitTargetDataWithReusableActor(
...
{
	UESAT_WaitTargetDataUsingActor* MyObj = NewAbilityTask<
		UESAT_WaitTargetDataUsingActor>(OwningAbility, TaskInstanceName);
...
void UESAT_WaitTargetDataUsingActor::Activate()
{
	if (IsPendingKill())
	{
		return;
...
void UESAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCallback(const FGameplayAbilityTargetDataHandle& Data,
...
void UESAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCancelledCallback()
{
	check(AbilitySystemComponent);
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
void UESAT_WaitTargetDataUsingActor::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& Data)
{
	check(AbilitySystemComponent);
	if (!Ability)
	{
...
void UESAT_WaitTargetDataUsingActor::OnTargetDataCancelledCallback(const FGameplayAbilityTargetDataHandle& Data)
{
	check(AbilitySystemComponent);
...
void UESAT_WaitTargetDataUsingActor::ExternalConfirm(bool bEndTask)
{
	check(AbilitySystemComponent);
	if (TargetActor)
	{
...
void UESAT_WaitTargetDataUsingActor::ExternalCancel()
{
	check(AbilitySystemComponent);
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
void UESAT_WaitTargetDataUsingActor::InitializeTargetActor() const
{
	check(TargetActor);
	check(Ability);
	...
	TargetActor->TargetDataReadyDelegate.AddUObject(const_cast<UESAT_WaitTargetDataUsingActor*>(this),
	                                                &UESAT_WaitTargetDataUsingActor::OnTargetDataReadyCallback);
	TargetActor->CanceledDelegate.AddUObject(const_cast<UESAT_WaitTargetDataUsingActor*>(this),
	                                         &UESAT_WaitTargetDataUsingActor::OnTargetDataCancelledCallback);
}

void UESAT_WaitTargetDataUsingActor::FinalizeTargetActor() const
{
	check(TargetActor);
	check(Ability);
...
void UESAT_WaitTargetDataUsingActor::RegisterTargetDataCallbacks()
{
	...
	if (!bIsLocallyControlled)
	{
		...
		if (!bShouldProduceTargetDataOnServer)
		{
			...
			AbilitySystemComponent->AbilityTargetDataSetDelegate(SpecHandle, ActivationPredictionKey).AddUObject(
				this, &UESAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCallback);
			AbilitySystemComponent->AbilityTargetDataCancelledDelegate(SpecHandle, ActivationPredictionKey).AddUObject(
				this, &UESAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCancelledCallback);
...
void UESAT_WaitTargetDataUsingActor::OnDestroy(bool AbilityEnded)
{
	if (TargetActor)
	{
		AESGATA_Trace* TraceTargetActor = Cast<AESGATA_Trace>(TargetActor);
		if (TraceTargetActor)
		{
...
bool UESAT_WaitTargetDataUsingActor::ShouldReplicateDataToServer() const
{
	if (!Ability || !TargetActor)
	{
		return false;
