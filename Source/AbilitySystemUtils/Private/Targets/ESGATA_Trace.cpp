...
void AESGATA_Trace::ResetSpread()
{
	bUseAimingSpreadMod = false;
	BaseSpread = 0.0f;
	AimingSpreadMod = 0.0f;
...
float AESGATA_Trace::GetCurrentSpread() const
{
...
void AESGATA_Trace::SetStartLocation(const FGameplayAbilityTargetingLocationInfo& InStartLocation)
{
	StartLocation = InStartLocation;
}

void AESGATA_Trace::SetShouldProduceTargetDataOnServer(bool bInShouldProduceTargetDataOnServer)
{
	ShouldProduceTargetDataOnServer = bInShouldProduceTargetDataOnServer;
}

void AESGATA_Trace::SetDestroyOnConfirmation(bool bInDestroyOnConfirmation)
{
	this->bDestroyOnConfirmation = bDestroyOnConfirmation;
}

void AESGATA_Trace::StartTargeting(UGameplayAbility* Ability)
{
...
void AESGATA_Trace::ConfirmTargetingAndContinue()
{
...
void AESGATA_Trace::CancelTargeting()
{
...
void AESGATA_Trace::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	DestroyReticleActors();
...
void AESGATA_Trace::Tick(float DeltaSeconds)
{
...
void AESGATA_Trace::LineTraceWithFilter(TArray<FHitResult>& OutHitResults, const UWorld* World,
...
void AESGATA_Trace::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params,
...
bool AESGATA_Trace::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection, FVector AbilityCenter,
...
void AESGATA_Trace::StopTargeting()
{
	SetActorTickEnabled(false);
...
FGameplayAbilityTargetDataHandle AESGATA_Trace::MakeTargetData(const TArray<FHitResult>& HitResults) const
{
...
TArray<FHitResult> AESGATA_Trace::PerformTrace(AActor* InSourceActor)
{
...
AGameplayAbilityWorldReticle* AESGATA_Trace::SpawnReticleActor(FVector Location, FRotator Rotation)
{
	if (ReticleClass)
	{
...
void AESGATA_Trace::DestroyReticleActors()
{
